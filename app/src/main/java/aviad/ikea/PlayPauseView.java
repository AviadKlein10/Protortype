...
/**
 * Created by Aviad on 23/02/2017.
 */
...
public class PlayPauseView extends FrameLayout {
    ...
    public PlayPauseView(Context context, AttributeSet attrs) {
        ...
        mBackgroundColor = ContextCompat.getColor(context, R.color.play_color);
        mPaint.setAntiAlias(true);
        mPaint.setStyle(Paint.Style.FILL);
        mDrawable = new PlayPauseDrawable(context);
        mDrawable.setCallback(this);

        mPauseBackgroundColor = ContextCompat.getColor(context, R.color.pause_color);
        mPlayBackgroundColor = ContextCompat.getColor(context, R.color.play_color);
    }


    public void setPlayPauseAnimationDuration(long newDuration){
        this.PLAY_PAUSE_ANIMATION_DURATION = newDuration;
    }
    @Override
    ...
    public void setIsPlay(boolean play, long duration){
        if (mAnimatorSet != null) {
            mAnimatorSet.cancel();
        }

        mAnimatorSet = new AnimatorSet();
        final ObjectAnimator colorAnim = ObjectAnimator.ofInt(this, COLOR, play ? mPauseBackgroundColor : mPlayBackgroundColor);
        colorAnim.setEvaluator(new ArgbEvaluator());

        final Animator pausePlayAnim = mDrawable.getPlayAnimator(play);
            mAnimatorSet.setInterpolator(new DecelerateInterpolator());
            mAnimatorSet.setDuration(PLAY_PAUSE_ANIMATION_DURATION);
            mAnimatorSet.playTogether(colorAnim, pausePlayAnim);
            mAnimatorSet.start();
